...
const ComparePage = () => {
  return (
    <div className="min-h-screen bg-slate-50 font-sans pt-20">
       ...
       <section className="bg-white pb-20 pt-16 border-b border-slate-200">
           <div className="max-w-7xl mx-auto px-4 text-center">
               <span className="text-blue-600 font-bold tracking-wider text-sm uppercase mb-4 block">Competitive Landscape</span>
               <h1 className="text-4xl md:text-6xl font-extrabold text-slate-900 mb-6">
                   Hyring vs. The Rest
               </h1>
               <p className="text-xl text-slate-600 max-w-3xl mx-auto mb-10">
                   Discover why Hyring is the smarter choice for modern recruitment. 
                   <span className="text-slate-900 font-semibold block mt-2">2X faster screening compared to others.</span>
               </p>
           </div>
       </section>
       ...
       <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
           <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-200">
               ...
               <div className="grid grid-cols-12 bg-slate-50 border-b border-slate-200 p-6 items-center">
                   <div className="col-span-4 font-bold text-slate-500 uppercase tracking-widest text-sm">Feature</div>
               ...
               {[
                   ...
                   <div key={i} className={`grid grid-cols-12 p-6 items-center ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} border-b border-slate-100 last:border-0 hover:bg-blue-50/20 transition-colors`}>
                       <div className="col-span-4 font-semibold text-slate-700">{row.feature}</div>
                       <div className="col-span-4 text-center flex justify-center">
                           {row.hyring === true ? (
                               <div className="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center text-green-600"><Check size={20} strokeWidth={3} /></div>
                           ) : (
                               <span className="font-bold text-slate-900 text-lg">{row.hyring}</span>
                           )}
                       </div>
                       <div className="col-span-4 text-center flex justify-center opacity-60">
                            {row.others === false ? (
                                <div className="w-8 h-8 bg-red-100 rounded-full flex items-center justify-center text-red-500"><X size={20} strokeWidth={3} /></div>
                            ) : row.others === true ? (
                                <div className="w-8 h-8 bg-slate-200 rounded-full flex items-center justify-center text-slate-500"><Check size={20} /></div>
                            ) : (
                                <span className="font-medium text-slate-500">{row.others}</span>
                            )}
                       </div>
                   </div>
               ))}
