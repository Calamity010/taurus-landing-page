import { motion } from 'framer-motion';
import { Globe, Share2, ChevronRight } from 'lucide-react';
import { Link } from 'react-router-dom';

const sections = [
  {
    number: '1',
    title: 'Employee Responsibility',
    intro: 'Employees must not:',
    items: [
      'Share confidential hiring data.',
      'Reveal AI ranking systems.',
      'Post misleading job offers.',
      'Engage in online arguments or negative commentary about competitors.',
      'Any mention of clients, partners, or candidates requires prior approval from the Communications team.',
    ],
  },
  {
    number: '2',
    title: 'Brand Representation',
    intro: 'Only authorized spokesperson can speak to media.',
    items: [],
  },
  {
    number: '3',
    title: 'Personal Branding',
    intro: 'Allowed, provided it does not:',
    items: [
      'Conflict with company interests.',
      'Mislead candidates or recruiters.',
    ],
  },
  {
    number: '4',
    title: 'Monitoring and Compliance',
    intro: null,
    items: [
      'Taurus AI reserves the right to monitor public social media activity that references the company.',
      'Employees found violating this policy may face disciplinary action.',
      'Regular training sessions will be conducted to educate employees on social media best practices.',
    ],
  },
  {
    number: '5',
    title: 'Reporting Violations',
    intro: null,
    items: [
      'Employees who observe policy violations on social media should report them to HR or the Communications team.',
      'Whistleblower protection applies to good-faith reports.',
    ],
  },
  {
    number: '6',
    title: 'Consequences',
    intro: 'Violations of the Social Media Policy may result in warnings, mandatory training, suspension, or termination depending on the severity and impact of the breach.',
    items: [],
  },
];

const SocialMediaPolicyPage = () => {
  return (
    <div className="min-h-screen bg-background font-sans pt-20">
      {/* Hero */}
      <section className="bg-background border-b border-white/10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-blue-900/30 text-blue-400 rounded-full text-sm font-bold mb-6 border border-blue-500/20">
            <Globe size={14} />
            Knowledge Bank
          </div>
          <nav className="flex items-center justify-center gap-2 text-sm text-slate-400 mb-6">
            <Link to="/resources/policies" className="hover:text-white transition-colors">Policies</Link>
            <ChevronRight size={14} />
            <span className="text-white">Social Media Policy</span>
          </nav>
          <div className="flex items-center justify-center gap-3 mb-4">
            <div className="p-3 bg-blue-600/20 rounded-2xl border border-blue-500/30">
              <Share2 className="w-8 h-8 text-blue-400" />
            </div>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 font-display">Social Media Policy</h1>
          <p className="text-xl text-slate-400 max-w-2xl mx-auto leading-relaxed">
            Guidelines governing how Taurus AI employees engage on social media platforms — protecting the brand, candidates, and company data.
          </p>
        </div>
      </section>

      {/* Content */}
      <section className="py-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-8">
            {sections.map((sec, idx) => (
              <motion.div
                key={sec.number}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: idx * 0.07 }}
                className="bg-card rounded-2xl border border-white/10 p-8 hover:border-blue-500/30 transition-all"
              >
                <h2 className="text-2xl font-bold text-white mb-4 flex items-center gap-3">
                  <span className="flex-shrink-0 w-9 h-9 rounded-full bg-blue-600/20 border border-blue-500/30 flex items-center justify-center text-blue-400 font-bold text-sm">
                    {sec.number}
                  </span>
                  {sec.title}
                </h2>
                {sec.intro && (
                  <p className="text-slate-300 mb-4 leading-relaxed">{sec.intro}</p>
                )}
                {sec.items.length > 0 && (
                  <ul className="space-y-2">
                    {sec.items.map((item, i) => (
                      <li key={i} className="flex items-start gap-3 text-slate-400">
                        <span className="mt-2 w-1.5 h-1.5 rounded-full bg-blue-400 flex-shrink-0" />
                        {item}
                      </li>
                    ))}
                  </ul>
                )}
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="bg-secondary py-16 text-center text-white">
        <div className="max-w-4xl mx-auto px-4">
          <h2 className="text-3xl font-bold mb-4">Have Questions About Our Policies?</h2>
          <p className="text-slate-300 mb-8 max-w-2xl mx-auto">
            Reach out to HR for clarifications or to report any violations confidentially.
          </p>
          <Link
            to="/contact-us"
            className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full font-medium transition-colors"
          >
            Contact HR <ChevronRight size={16} />
          </Link>
        </div>
      </section>
    </div>
  );
};

export default SocialMediaPolicyPage;
